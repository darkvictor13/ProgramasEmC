...
int LerDados (Veiculo *vet, int *num) {
    ...
    *num = 0;
    ...
    //abrir o arquivo e verificar se realmente foi possível abrir
    arq = fopen("veiculos2.txt", "r");
    if(arq == NULL){
...
void CalcIPVA (Veiculo *vet, int tam, float aliq[3][4]) {
    int i, j;
...
void ImprimeIPVAS(Veiculo *vet, int num) {
    int i;
    FILE *arq;
